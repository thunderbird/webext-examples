 ...
 * a persistent listener and will not wake up the background, after the tab stays
 * open longer then the background idle timeout without the user interacting with
 * it.
 * 
 * This example extension was updated using a global persistent `runtime.onMessage`
 * event listener.
 * 
 * Alternatively, the tab could send a periodic ping to the `runtime.onMessage`
 * listener in the background to keep the background busy. The `awaitPopup` example
 * extension has been updated using the periodic ping approach.
 */
