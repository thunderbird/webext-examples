        ...
        ResourceUrl: {
          register(namespace, folder) {
            ...
            if (resProto.hasSubstitution(namespace)) {
              throw new ExtensionError(`There is already a resource:// url for the namespace "${namespace}". If this error is thrown during MV3 background wake-up, there is currently no way to prevent this. There is no state we can query, if background was executed due to install, enable, startup or wakeup (we only have onInstalled and onStartup events).`);
            }
