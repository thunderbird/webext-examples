...
async function pdfPreview(url, canvas, nbP) { // RH : We preview the pdf in the new window.
...
async function generateThumbs() {
    ...
    var messageId = await parseInt(url.searchParams.get("messageId")); // rh :We get the messageID in the parameters of the URL.
    var filename = await url.searchParams.get("filename"); // RH : We get the partName in the parameters of the URL.
    var numPage = await parseInt(url.searchParams.get("numPage")); // RH : We get the number of the pages in the parameters of the URL.
    let attachments = await messenger.messages.listAttachments(messageId);
    for (let attachment of attachments) {
        if (attachment.partName == filename) { // RH : If the partname get == the partname of the for
            ...
            var id = `attachmentElement_${messageId}_${attachment.partName}`;
            var t = document.querySelector('#attachmentTemplate');
            ...
            var attachmentElement = document.getElementById(id);
            ...
            if (attachment.contentType.toLowerCase().startsWith("image/"))
                attachmentElement.querySelector("img").src = attachment.url; // RH : On met notre image attachment dans une image.
            else
                attachmentElement.querySelector("img").style.display = "none";

            if (attachment.contentType.toLowerCase() == "application/pdf") {
                pdfPreview(attachment.url, attachmentElement.querySelector("canvas"), numPage); // RH : On appelle la fonction pdfPreview.
            } else {
...
async function clickHandler(e) {
    ...
    switch (e.target.getAttribute("action")) {
        ...
        case "download": // RH : For download the page.
            {
